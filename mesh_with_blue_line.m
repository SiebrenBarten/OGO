...
theta = linspace(0, 90, 100);  % [degrees], Matrix with range of angles between the primary (at 0 degrees)
...
% Evaluate the polynomial over a range of V_f values
V_f_poly = linspace(0, V_f_lim, 100);
theta_poly = slope_2 * V_f_poly.^2 + slope * V_f_poly + intercept;


% % Plot the polynomial equation over the surface plot
% plot3(V_f_poly, theta_poly, E_effective(1,:), 'b-', 'LineWidth', 3);
% 
% % Evaluate the polynomial at the intersection points
% theta_poly_eval = slope_2 * V_f_L.^2 + slope * V_f_L + intercept;
% 
% % Calculate the root mean square error (RMSE)
% rmse = sqrt(mean((theta_L - theta_poly_eval).^2));
% 
% fprintf('The root mean square error (RMSE) is %.4f\n', rmse);
